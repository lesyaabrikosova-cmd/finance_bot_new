...
class UserSettings:
    ...
    # Только явно выбранные пользователем семейства цветов. Отсутствие записи
    # означает автоматический цвет и не показывается в карточке типа дохода.
    income_type_manual_color_families: Dict[str, int] = field(default_factory=dict)
