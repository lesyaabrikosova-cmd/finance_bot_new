...
async def show_intro(
    ...
    await message.answer(
        ...
        "Этот бот не просто записывает, "
        "куда вы потратили деньги.\n\n"

        "Его задача — <b>заранее распределять каждое "
        "поступление</b> между обязательными расходами, "
        "финансовой подушкой, целями, кредитами "
        "и инвестициями.\n\n"

        "Чтобы алгоритм работал именно под вашу жизнь, "
        ...
        "Никаких специальных финансовых знаний "
        "не требуется.\n\n"

        "⏱ Настройка состоит из нескольких небольших "
        "разделов. После неё вы увидите все введённые "
...
async def confirm_save(
    ...
    await callback.message.answer(
        ...
        "Теперь Аллокатор знает ваши обязательные "
        "расходы, резерв, кредиты, цели и стартовую "
        "финансовую позицию.\n\n"

        f"⚙️ Текущий режим: <b>{mode}</b>"
        f"{next_text}\n\n"

        "Следующий шаг — добавить первое поступление "
        "денег. Для этого мы подключим основной "
        "интерфейс бота."
    )
