...
async def start(
    ...
    if existing is not None:
        ...
        await message.answer(
            ...
            "Изменить отдельные параметры можно в Настройках.\n"
            "Чтобы пройти настройку с нуля, нажмите «Настроить заново».",
...
async def show_intro(
    ...
    caption = (
        "Я — Богатый Алхимик — ваш финансовый аллокатор.\n\n"
        "Я не буду заставлять вас записывать каждую чашку кофе и разбираться, "
        "куда вчера исчезли деньги.\n\n"
        "Главный вопрос здесь другой: что делать с деньгами в тот момент, "
        "когда они только пришли?"
    )
...
async def intro_step_2(
    ...
    caption = (
        ...
        "Он заранее распределяет каждый доход по финансовым «конвертам».\n"
        "Часть из них очевидна. Другие вы, возможно, вообще не стали бы создавать сами — "
        "хотя именно они делают финансовую систему устойчивой."
    )
...
async def intro_step_3(
    ...
    caption = (
        "Секрет философского камня прост: чтобы начать богатеть, сначала нужно "
        "научиться управлять тем, что уже зарабатываешь.\n\n"
        "Можно получать миллион и тратить его в тот же месяц. Больше зарабатывать "
        "в такой ситуации — не всегда решение.\n\n"
        "Часто проблема не в размере дохода, а в том, как человек им распоряжается.\n\n"
        "А это действительно сложно, ведь научно доказано, что в финансовых вопросах "
        "человеческий мозг склонен принимать иррациональные решения.\n"
        "Финансовое планирование — это работа с вероятностями. Мы не знаем, что случится "
        "завтра, но можем заранее подготовить деньги к тому, что жизнь пойдёт не по плану."
    )
...
async def intro_step_4(
    ...
    caption = (
        "Волшебная таблетка существует. Но её нужно приготовить.\n\n"
        "Чтобы Аллокатор работал именно под вашу жизнь, сначала создадим финансовый профиль.\n\n"
        "Я буду задавать вопросы по одному и объяснять, что означает каждая цифра и где её взять.\n\n"
