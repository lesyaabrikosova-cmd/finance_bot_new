...
class GoalOverflowTests(unittest.TestCase):
    def test_caps_target_and_splits_overflow_between_remaining_positions(self):
        ...
        self.assertEqual(result, {
            "Цели:Ноутбук": D("2000"),
            "Цели:Поездка": D("3333.33"),
            "Цели:Подарки": D("1833.33"),
            "Цели:Хотелки": D("2833.34"),
        })
    ...
    def test_paused_positions_count_toward_creation_limits(self):
        goals = [
            Goal(f"Цель {index}", 0, status="paused", target_amount=1000)
            for index in range(5)
        ]
        goals += [
            Goal(f"Сундук {index}", 0, status="paused", position_type="chest")
            for index in range(3)
        ]
        a = allocator(goals)

        self.assertIn("5 текущих Целей", a.position_creation_error("goal"))
        self.assertIn("8 текущих", a.position_creation_error("chest"))

    def test_completed_goal_releases_current_creation_slot(self):
        goals = [
            Goal(f"Цель {index}", 0, status="paused", target_amount=1000)
            for index in range(4)
        ]
        goals.append(Goal("Готовая", 0, status="completed", target_amount=1000))
        goals.append(Goal("Сундук", 100, position_type="chest"))
        a = allocator(goals)

        self.assertIsNone(a.position_creation_error("goal"))
    ...
    def test_pause_routes_released_share_only_to_system_chest(self):
        goal = Goal("Отпуск", 30, target_amount=1000)
        regular = Goal("Техника", 20, position_type="chest")
        fallback = Goal("Подарки", 50, position_type="chest", is_system_chest=True)
        a = allocator([goal, regular, fallback])

        a.pause_position(goal)

        self.assertEqual([item.percentage for item in a.settings.goals], [D(0), D(20), D(80)])
        self.assertTrue(a.settings.allocation_needs_review)

    def test_pause_can_send_released_share_to_selected_position(self):
        goal = Goal("Отпуск", 30, target_amount=1000)
        selected = Goal("Техника", 20, position_type="chest")
        fallback = Goal("Подарки", 50, position_type="chest", is_system_chest=True)
        a = allocator([goal, selected, fallback])

        a.pause_position_with_distribution(goal, recipient=selected)

        self.assertEqual([item.percentage for item in a.settings.goals], [D(0), D(50), D(50)])
        self.assertFalse(a.settings.allocation_needs_review)

    def test_pause_can_split_released_share_evenly_without_losing_rounding_remainder(self):
        goal = Goal("Отпуск", D("10"), target_amount=1000)
        first = Goal("Техника", D("30"), position_type="chest")
        second = Goal("Хотелки", D("30"), position_type="chest")
        fallback = Goal("Подарки", D("30"), position_type="chest", is_system_chest=True)
        a = allocator([goal, first, second, fallback])

        a.pause_position_with_distribution(goal, evenly=True)

        active_total = sum((item.percentage for item in a.settings.active_goals), D(0))
        self.assertEqual(active_total, D(100))
        self.assertEqual(first.percentage, D("33.33"))
        self.assertEqual(second.percentage, D("33.33"))
        self.assertEqual(fallback.percentage, D("33.34"))

    def test_reactivation_starts_at_zero_without_changing_existing_shares(self):
        waiting = Goal("Отпуск", 0, status="paused", target_amount=1000, previous_percentage=30)
        regular = Goal("Техника", 20, position_type="chest")
        fallback = Goal("Подарки", 80, position_type="chest", is_system_chest=True)
        a = allocator([waiting, regular, fallback])
    ...
    def test_only_active_position_is_automatic_hundred_without_warning(self):
        goal = Goal("Отпуск", 30, target_amount=1000)
        fallback = Goal("Подарки", 70, position_type="chest", is_system_chest=True)
        a = allocator([goal, fallback])

        a.pause_position(goal)

        self.assertEqual(fallback.percentage, D(100))
        self.assertFalse(a.settings.allocation_needs_review)

    def test_forecast_copy_and_real_allocation_share_status_behavior(self):
        source = allocator([
            Goal("Отпуск", 30, target_amount=10),
            Goal("Техника", 20, position_type="chest"),
            Goal("Подарки", 50, position_type="chest", is_system_chest=True),
        ])
        simulated = deepcopy(source)

        simulated_result = simulated.process_income(D(100), "Тест", tax_override=D(0))
        real_result = source.process_income(D(100), "Тест", tax_override=D(0))

        self.assertEqual(simulated_result.allocations, real_result.allocations)
        self.assertEqual(
            [(goal.status, goal.percentage) for goal in simulated.settings.goals],
            [(goal.status, goal.percentage) for goal in source.settings.goals],
        )
    ...
    def test_automatic_completion_splits_released_share_between_remaining_positions(self):
        finished = Goal("Готовая", 30, target_amount=10)
        regular = Goal("Техника", 20, position_type="chest")
        fallback = Goal("Подарки", 50, position_type="chest", is_system_chest=True)
        a = allocator([finished, regular, fallback])

        a._allocate_goals(D(100), {})

        self.assertEqual([item.percentage for item in a.settings.goals], [D(0), D(35), D(65)])
        self.assertFalse(a.settings.allocation_needs_review)

    def test_current_income_overflow_is_also_split_between_remaining_positions(self):
        finished = Goal("Готовая", 30, target_amount=10)
        another = Goal("Другая", 20, target_amount=1000)
        chest = Goal("Подарки", 50, position_type="chest", is_system_chest=True)
        a = allocator([finished, another, chest])

        split = a.split_goal_amount(D("100"))

        self.assertEqual(
            split,
            {"Готовая": D("10"), "Другая": D("30"), "Подарки": D("60")},
        )

    def test_manual_completion_splits_fraction_without_losing_remainder(self):
        finished = Goal("Готовая", D("10"), target_amount=100)
        first = Goal("Первая", D("30"), target_amount=1000)
        second = Goal("Техника", D("30"), position_type="chest")
        fallback = Goal("Подарки", D("30"), position_type="chest", is_system_chest=True)
        a = allocator([finished, first, second, fallback])

        a.complete_goal(finished)

        self.assertEqual(
            [item.percentage for item in a.settings.active_goals],
            [D("33.33"), D("33.33"), D("33.34")],
        )
        self.assertEqual(
            sum((item.percentage for item in a.settings.active_goals), D(0)),
            D(100),
        )
    ...
    def test_zero_weight_chests_share_to_kopecks_and_preserve_remainder(self):
        ...
        self.assertEqual(split, {"Первый": D("50.00"), "Второй": D("50.01")})
...
class ChestLifecycleTests(unittest.IsolatedAsyncioTestCase):
    async def test_new_active_position_starts_at_zero_and_marks_review(self):
        fallback = Goal("Подарки", 100, position_type="chest", is_system_chest=True)
        a = allocator([fallback])
        state = AsyncMock()
        state.get_data.return_value = {
            "goal_draft": {"name": "Техника", "position_type": "chest"}
        }
        message = SimpleNamespace(from_user=SimpleNamespace(id=42), answer=AsyncMock())
        with (
            patch("goals_manager.db") as db,
            patch("goals_manager.show_chests_list", new=AsyncMock()),
        ):
            db.load_allocator.return_value = a
            await persist_new_position(message, state)

        self.assertEqual(fallback.percentage, D(100))
        self.assertEqual(a.settings.goals[-1].percentage, D(0))
        self.assertTrue(a.settings.allocation_needs_review)

    async def test_main_manager_rejects_ninth_current_position(self):
        goals = [Goal(f"Цель {index}", 10, target_amount=1000) for index in range(4)]
        goals += [
            Goal(f"Сундук {index}", 15, position_type="chest")
            for index in range(4)
        ...
        self.assertEqual(len(a.settings.goals), 8)
        self.assertIn("8 текущих", message.answer.await_args_list[0].args[0])

    async def test_ninth_onboarding_position_is_rejected(self):
        class DraftState:
            def __init__(self):
                self.data = {
                    "goal_drafts": [
        ...
        self.assertEqual(len(state.data["goal_drafts"]), 8)

    async def test_sixth_onboarding_goal_is_rejected(self):
        class DraftState:
            def __init__(self):
                self.data = {
                    "goal_drafts": [
        ...
        self.assertEqual(len(state.data["goal_drafts"]), 5)
    ...
    async def test_completing_goal_splits_share_between_remaining_positions(self):
        a = allocator([Goal("Готовая", 60, target_amount=100), Goal("Другая", 20, target_amount=200),
                       Goal("Подарки", 20, position_type="chest", is_system_chest=True)])
        callback = SimpleNamespace(answer=AsyncMock(), message=SimpleNamespace(answer=AsyncMock()),
                                   from_user=SimpleNamespace(id=42), data="goalmanage:complete:yes:0")
        with patch("goals_manager.db") as db, patch("goals_manager.show_goals_list", new=AsyncMock()):
            db.load_allocator.return_value = a
            await complete_position(callback, AsyncMock())
        self.assertEqual([g.percentage for g in a.settings.goals], [D(0), D(50), D(50)])
        self.assertFalse(a.settings.allocation_needs_review)
        self.assertIn(
            "поровну распределена",
            callback.message.answer.await_args.args[0],
        )

    async def test_delete_routes_released_share_only_to_fallback(self):
        goal = Goal("Отпуск", 30, target_amount=100)
        regular = Goal("Техника", 20, position_type="chest")
        fallback = Goal("Подарки", 50, position_type="chest", is_system_chest=True)
        a = allocator([goal, regular, fallback])
        callback = SimpleNamespace(
            ...
            from_user=SimpleNamespace(id=42),
            data=f"goalmanage:delete:yes:{goal.uid}",
        )
        with (
            patch("goals_manager.db") as db,
            patch("goals_manager.show_goals_list", new=AsyncMock()),
        ):
            db.load_allocator.return_value = a
            await delete_position(callback, AsyncMock())

        self.assertEqual([item.percentage for item in a.settings.goals], [D(20), D(80)])
        self.assertTrue(a.settings.allocation_needs_review)

    async def test_confirmed_percentages_are_positive_hundred_and_clear_review(self):
        goal = Goal("Отпуск", 0, target_amount=100)
        fallback = Goal("Подарки", 100, position_type="chest", is_system_chest=True)
        paused = Goal("Позже", 15, status="paused", target_amount=100)
        a = allocator([goal, fallback, paused])
        a.settings.allocation_needs_review = True
        state = AsyncMock()
        state.get_data.return_value = {
            "goal_percentage_uids": [goal.uid, fallback.uid],
            "goal_percentages_pending": ["40", "60"],
        }
        callback = SimpleNamespace(
            answer=AsyncMock(),
            message=SimpleNamespace(answer=AsyncMock()),
            from_user=SimpleNamespace(id=42),
            data="goalmanage:percent:save",
        )
        with (
            patch("goals_manager.db") as db,
            patch("goals_manager.show_allocation", new=AsyncMock()),
        ):
            db.load_allocator.return_value = a
            await confirm_percentages(callback, state)

        self.assertEqual([item.percentage for item in a.settings.active_goals], [D(40), D(60)])
        self.assertEqual(paused.percentage, D(0))
        self.assertFalse(a.settings.allocation_needs_review)

    async def test_percentage_save_rejects_zero_active_share(self):
        goal = Goal("Отпуск", 0, target_amount=100)
        fallback = Goal("Подарки", 100, position_type="chest", is_system_chest=True)
        a = allocator([goal, fallback])
        a.settings.allocation_needs_review = True
        ...
        state.get_data.return_value = {
            "goal_percentage_uids": [goal.uid, fallback.uid],
            "goal_percentages_pending": ["0", "100"],
        }
        callback = SimpleNamespace(
            answer=AsyncMock(),
            message=SimpleNamespace(answer=AsyncMock()),
    ...
    async def test_balance_edit_records_adjustment_without_rewriting_income(self):
        fallback = Goal("Подарки", 100, position_type="chest", is_system_chest=True, balance=50)
        a = allocator([fallback])
        state = AsyncMock()
        state.get_data.return_value = {"edit_goal_uid": fallback.uid}
        message = SimpleNamespace(
            text="75",
            answer=AsyncMock(),
            from_user=SimpleNamespace(id=42),
        ...
        with (
            patch("goals_manager.db") as db,
            patch("goals_manager.show_position_card", new=AsyncMock()),
        ):
            db.load_allocator.return_value = a
            await save_edited_balance(message, state)

        self.assertEqual(a.state.goal_balances[fallback.name], D(75))
        db.save_operation.assert_called_once()
        self.assertEqual(db.save_operation.call_args.args[1], "envelope_balance_adjustment")
