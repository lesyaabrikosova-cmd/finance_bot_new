...
class GoalAndChestDeletionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_overflow_chest_can_be_deleted_when_another_chest_is_active(self):
        allocator = make_allocator(
            goals=[
                self._system_chest(),
                Goal("Подарки", D("50"), position_type="chest", uid="gifts"),
        ...
        self.assertEqual([goal.name for goal in allocator.settings.goals], ["Подарки"])
        self.assertEqual(allocator.state.goal_balances, {"Подарки": D("100")})
        self.assertTrue(allocator.settings.goals[0].is_system_chest)
    ...
    async def test_overflow_chest_can_be_paused_when_another_chest_is_active(self):
        """The internal fallback role follows the remaining active chest."""
        allocator = make_allocator(
            goals=[
                self._system_chest(),
                Goal("Подарки", D("50"), position_type="chest", uid="gifts"),
        ...
        source, recipient = allocator.settings.goals
        allocator.pause_position_with_distribution(source, recipient=recipient)

        self.assertEqual(source.status, "paused")
        self.assertTrue(recipient.is_system_chest)
