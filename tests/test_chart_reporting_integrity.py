...
from charts import CHART_BACKGROUND, chart_items, make_chart, report_fallback_text, send_chart_report, wrap_legend_label


class ChartLegendIntegrityTests(unittest.TestCase):
    def test_adjacent_slices_have_no_background_coloured_separator(self):
        from PIL import Image

        data = make_chart(
            {"Первый": D("50"), "Второй": D("50")},
            "БАЛАНСЫ",
            colors={"Первый": "#B42335", "Второй": "#F47A7A"},
            preserve_order=True,
        )
        image = Image.open(BytesIO(data))
        self.assertNotEqual(image.getpixel((540, 700)), tuple(bytes.fromhex(CHART_BACKGROUND[1:])))
        self.assertNotEqual(image.getpixel((520, 700)), tuple(bytes.fromhex(CHART_BACKGROUND[1:])))
...
class ChartFallbackIntegrityTests(unittest.IsolatedAsyncioTestCase):
    async def test_empty_report_text_sends_photo_without_caption(self):
        navigation = object()
        ...
        self.assertIsNone(message.answer_photo.await_args.kwargs["caption"])
        self.assertIs(message.answer_photo.await_args.kwargs["reply_markup"], navigation)
