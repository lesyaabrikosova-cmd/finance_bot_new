...
class GoalMathTests(unittest.TestCase):
    ...
    def test_each_position_keeps_at_least_one_percent(self):
        ...
        self.assertEqual(
            ...
            [Decimal("30.5"), Decimal("69.5")],
        )
        with self.assertRaises(ValueError):
            sequential_goal_percentages([Decimal("30.555")], 2)

    def test_setup_capacity_ignores_all_protection_choice(self):
        allocator = allocator_with(
            [Goal("Отпуск", 100)],
            tax_rate=Decimal("0"),
        )
        allocator.settings.force_majeure_months = Decimal("10")
        allocator.settings.protective_stage_c_strategy = "protection"
        allocator.settings.set_brackets(20, 25, 30, 35)
        capacity = allocator.allocation_setup_capacity_range()
        self.assertEqual(capacity["minimum"], Decimal("21000.00"))
        self.assertEqual(capacity["maximum"], Decimal("21000.00"))

    def test_piecework_setup_uses_exact_average_and_only_tax_rates_form_range(self):
        settings = UserSettings(
            has_debts=False,
            ...
            income_type_tax_rates={"Без налога": Decimal("0"), "НПД": Decimal("6")},
            goals=[Goal("Отпуск", 100)],
        )
        allocator = FinancialAllocator(
            settings,
            AllocatorState(pillow_stabilizer=Decimal("110000")),
        )
        capacity = allocator.allocation_setup_capacity_range()
        self.assertEqual(capacity["income_minimum"], Decimal("180000.00"))
        self.assertEqual(capacity["income_maximum"], Decimal("180000.00"))
        self.assertEqual(capacity["minimum"], Decimal("21023.33"))
        self.assertEqual(capacity["maximum"], Decimal("28583.33"))

    def test_stable_setup_uses_guaranteed_income_and_level_three_half_share(self):
        settings = UserSettings(
            has_debts=False,
            profile_type="stable",
            ...
            income_type_tax_rates={"Без налога": Decimal("0")},
            goals=[Goal("Отпуск", 100)],
        )
        allocator = FinancialAllocator(settings, AllocatorState())

        self.assertEqual(allocator.active_mode(), 3)
        capacity = allocator.allocation_setup_capacity_range()
        self.assertEqual(capacity["income_minimum"], Decimal("180000.00"))
        self.assertEqual(capacity["minimum"], Decimal("14291.67"))
        self.assertEqual(capacity["maximum"], Decimal("14291.67"))

    def test_cyclic_setup_uses_cycle_average_and_monthly_contract_obligations(self):
        settings = UserSettings(
            has_debts=False,
            profile_type="cyclic",
            employment_type="Наёмный",
            ...
            bracket_c=Decimal("30"),
            force_majeure_months=Decimal("3"),
            income_work_months=Decimal("1"),
            income_gap_months=Decimal("1"),
            contract_obligations={"Связь": Decimal("24000")},
            income_type_tax_rates={"Без налога": Decimal("0")},
            goals=[Goal("Отпуск", 100)],
        )
        allocator = FinancialAllocator(settings, AllocatorState())
        ...
        capacity = allocator.allocation_setup_capacity_range()
        self.assertEqual(capacity["income_minimum"], Decimal("180000.00"))
        self.assertEqual(capacity["minimum"], Decimal("10091.67"))
        self.assertEqual(capacity["maximum"], Decimal("10091.67"))
