...
from dashboard import (
    ...
    income_color_types,
    income_month_analysis_keyboard,
...
class IncomeHistoryTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_card_and_distribution_keep_note_and_group_order(self):
        ...
        self.assertLess(text.index("📈"), text.index("❤️ Квартира"))
    ...
    async def test_month_detail_and_distribution_keep_selected_month(self):
        ...
        self.assertIn(
            ...
            detail_message.answer.await_args.args[0],
        )
    ...
    def test_month_analysis_uses_arrows_instead_of_a_legacy_income_button(self):
        markup = income_month_analysis_keyboard(2025, 3)
        buttons = [button for row in markup.inline_keyboard for button in row]
        texts = [button.text for button in buttons]
        callbacks = [button.callback_data for button in buttons]
        self.assertEqual(texts[:2], ["<", ">"])
        self.assertIn("incomeanalysis:monthnav:2025:2", callbacks)
        self.assertIn("incomeanalysis:monthnav:2025:4", callbacks)
        self.assertNotIn("Поступления марта", texts)
    ...
    async def test_history_page_uses_period_navigation_symbols(self):
        ...
        self.assertIn(">", [button.text for button in first_markup.inline_keyboard[-2]])
        self.assertIn("<", [button.text for button in second_markup.inline_keyboard[-2]])
