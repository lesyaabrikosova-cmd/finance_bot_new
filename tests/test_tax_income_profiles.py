...
class TaxIncomeProfileMenus(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_patent_income_confirmation_explains_zero_withholding(self):
        ...
        with patch("income.db") as income_db:
            income_db.load_allocator.return_value = current
            income_db.load_tax_obligations.return_value = [{
                "tax_type": "Патент",
                "object_name": "Музыкальные занятия",
            }]
    ...
    async def test_income_confirmation_without_patent_shows_zero_tax(self):
        message = SimpleNamespace(answer=AsyncMock())
        state = MemoryState(
            income_amount="3700",
            income_type="Частник",
            income_date="2026-09-16",
            tax_override=None,
            income_flow_id="flow-1",
        )
        with patch("income.db") as income_db:
            income_db.load_allocator.return_value = allocator()
            await show_income_confirmation(message, state, 42)

        text = message.answer.await_args.args[0]
        self.assertIn("🏛️ Налог — 0", text)
        self.assertNotIn("Патент копится отдельно", text)

    async def test_stale_patent_binding_without_patent_shows_zero_tax(self):
        current = allocator()
        current.settings.income_type_tax_rates = {"Частник": Decimal("0")}
        current.settings.income_type_tax_profiles = {
            "Частник": "ИП · ПСН · Старый патент · по плану",
        }
        message = SimpleNamespace(answer=AsyncMock())
        state = MemoryState(
            income_amount="3700",
