...
class ModeTransitionTests(unittest.TestCase):
    ...
    def test_profile_specific_mode_scales(self):
        ...
        self.assertEqual(cyclic.profile_mode_total, 6)
    ...
    def test_cyclic_reserves_cannot_substitute_for_each_other(self):
        allocator = FinancialAllocator(
            UserSettings(
                has_debts=False,
                employment_type="Фрилансер",
        ...
        # Для двух оставшихся периодов Фонду зарплаты нужны 80 000 и 100 000 ₽.
        # Деньги Подушки не могут преждевременно выдать кубок за Фонд зарплаты.
        self.assertEqual(allocator.protective_capital_balance, D("100000"))
        self.assertEqual(allocator.active_mode(), 3)
        self.assertEqual(allocator.remaining_to_profile_transition(), D("30000"))
    ...
    def test_cyclic_employee_gets_salary_fund_without_stabilizer(self):
        ...
        self.assertFalse(allocator.settings.needs_stabilizer)
        ...
        self.assertEqual(allocator.profile_mode_total, 6)
    ...
    def test_cyclic_income_fills_salary_fund_before_force_majeure(self):
        ...
        self.assertNotIn("Стабилизатор дохода", result.allocations)
        self.assert_money(allocator.state.pillow_stabilizer, "0")

    def test_cyclic_large_income_uses_salary_fund_then_pillow_without_stabilizer(self):
        settings = UserSettings(
            has_debts=False,
            employment_type="Наёмный",
            profile_type="cyclic",
        ...
        allocator = FinancialAllocator(
            settings,
            AllocatorState(life_balance=D("200"), household_reserve_progress=D("100")),
        )

        result = allocator.process_income(D("1000"), "Контракт", tax_override=D("0"))

        self.assert_money(allocator.state.intercontract_reserve, "200")
        self.assert_money(allocator.state.pillow_force_majeure, "100")
        self.assert_money(allocator.state.pillow_stabilizer, "0")
        self.assertNotIn("Стабилизатор дохода", result.allocations)
        self.assertEqual(allocator.active_mode(), 6)

    def test_cyclic_first_distribution_has_no_stabilizer_account(self):
        allocator = FinancialAllocator(UserSettings(
            has_debts=False,
            employment_type="Наёмный",
            profile_type="cyclic",
        ...
        self.assertNotIn("Стабилизатор дохода", allocator.first_distribution_balances())
        self.assertNotIn("Стабилизатор дохода", allocator.apply_first_distribution(D("1000")))
        self.assert_money(allocator.state.pillow_stabilizer, "0")
