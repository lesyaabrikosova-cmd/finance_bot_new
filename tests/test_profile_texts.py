...
from dashboard import income_distribution_table, period_balance_chart
...
from settings_editor import (
    show_profile_income_settings,
    show_reserves_settings,
    show_settings_actions,
    show_settings_menu,
    show_system_settings,
)
from period import (
    ask_extend_intercontract_period,
    period_remainder_amount_save,
    show_new_period_confirmation,
)
...
class ProfileTextTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_settings_actions_stay_behind_overview_button(self):
        ...
        self.assertEqual(
            buttons,
            "Профиль и доход Стоимость жизни Долги и кредиты Резервы Налоги "
            "Цели и Сундуки Бракеты Расходы к дате Финансовый архетип "
            "Системные настройки ← Главное меню",
        )

    async def test_profile_income_contains_financial_profile_controls(self):
        msg = message()
        with patch("settings_editor.db") as db:
            db.load_allocator.return_value = make_allocator("piecework")
            await show_profile_income_settings(msg, 42)

        buttons = button_texts(msg.answer.await_args.kwargs["reply_markup"])
        self.assertIn("Профиль: Сдельный", buttons)
        self.assertIn("Средний доход", buttons)
        self.assertIn("Типы доходов", buttons)
        self.assertNotIn("Финансовый архетип", buttons)

    async def test_reserve_settings_only_show_applicable_entities(self):
        expectations = {
            "stable": (False, False),
            "piecework": (True, False),
            "cyclic": (False, True),
        }
        for profile, (stabilizer, salary_fund) in expectations.items():
            msg = message()
            with patch("settings_editor.db") as db:
                db.load_allocator.return_value = make_allocator(profile)
                await show_reserves_settings(msg, 42)
            buttons = button_texts(msg.answer.await_args.kwargs["reply_markup"])
            self.assertEqual("Стабилизатор" in buttons, stabilizer)
            self.assertEqual("Фонд зарплаты" in buttons, salary_fund)

    async def test_system_settings_keep_destructive_actions_nested_and_plain(self):
        msg = message()
        with patch("settings_editor.db") as db:
            db.load_allocator.return_value = make_allocator("stable")
            await show_system_settings(msg, 42)

        buttons = button_texts(msg.answer.await_args.kwargs["reply_markup"])
        self.assertIn("Настроить профиль заново", buttons)
        self.assertIn("Сбросить все данные", buttons)
    ...
    async def test_settings_only_offer_applicable_reserves(self):
        for profile in ("stable", "piecework", "cyclic"):
            ...
            self.assertEqual("Стабилизатор" in combined, profile == "piecework")
    ...
    async def test_cyclic_maximum_level_offers_investments_without_stabilizer(self):
        allocator = make_allocator("cyclic")
        allocator.state.intercontract_reserve = allocator.intercontract_current_limit
        ...
        with patch("period.db") as db:
            ...
            await period_remainder_amount_save(msg, state)

        buttons = button_texts(msg.answer.await_args.kwargs["reply_markup"])
        self.assertIn("Инвестиции", buttons)
        self.assertNotIn("Стабилизатор", buttons)

    def test_cyclic_reports_hide_legacy_stabilizer_flows(self):
        allocator = make_allocator("cyclic")
        legacy_allocations = {
            "Фонд Зарплаты": D("100"),
            "Стабилизатор дохода": D("50"),
        }

        values, _ = period_balance_chart(allocator, legacy_allocations)
        self.assertIn("Фонд Зарплаты", values)
        self.assertNotIn("Стабилизатор", values)

        text = income_distribution_table({
            "payload": {"allocations": legacy_allocations, "tax": "0"},
        }, allocator)
        self.assertIn("Фонд Зарплаты", text)
        self.assertNotIn("Стабилизатор", text)

    async def test_cyclic_break_extension_never_offers_stabilizer(self):
        allocator = make_allocator("cyclic")
        ...
        with patch("period.db") as db:
            db.load_allocator.return_value = allocator
            await ask_extend_intercontract_period(callback)

        text = msg.answer.await_args.args[0]
        self.assertIn("Фонда Зарплаты", text)
        self.assertIn("Подушки", text)
        self.assertNotIn("Стабилизатор", text)
