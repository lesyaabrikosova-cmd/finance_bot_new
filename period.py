...
def _fund_currency_text(allocator) -> str:
    ...
    return (
        ...
        "Курс фиксируется для расчётного периода. Ежедневные колебания не меняют уровень. "
...
async def fund_currency_forecast_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "<b>ПРОГНОЗ ОБМЕНА</b>\n\nВыберите валюту. Прогноз ничего не спишет и не изменит уровень.",
...
async def show_new_period_confirmation(
    ...
    await message.answer(
        ...
        "⚙️ настройки\n"
        "📜 история операций\n\n"
        + ("Фонд Зарплаты и общий доход текущего финансового цикла тоже сохраняются.\n\n"
...
async def period_remainder_amount_save(message: Message, state: FSMContext):
    ...
    if allocator.settings.active_goals and active_mode >= goals_mode:
